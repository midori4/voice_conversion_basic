...
if __name__ == "__main__":
    ...
    features = ["mcep", "f0"] if traindata else ["mcep"]
    for sp in ["X", "Y"]:
    ...
    if traindata:
        ...
        # Perform alignment and save aligned mel cepstrum
        _, path = dtw(mcep1, mcep2)
        path1 = list(map(lambda x: x[0], path))
        path2 = list(map(lambda x: x[1], path))
        aligned_mcep1, aligned_mcep2 = mcep1[path1].astype(np.float32), mcep2[path2].astype(np.float32)
        mcep_dir1 = join(data_dir, "X", "mcep")
        mcep_dir2 = join(data_dir, "Y", "mcep")
        aligned_mcep1.tofile(join(mcep_dir1, splitext(basename(source_wav))[0] + ".mcep"))
        aligned_mcep2.tofile(join(mcep_dir2, splitext(basename(target_wav))[0] + ".mcep"))
    else:
        # Perform alignment and save aligned mel cepstrum
        _, path = dtw(mcep1, mcep2)
        path1 = list(map(lambda x: x[0], path))
