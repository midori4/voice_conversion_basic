...
class DTWAligner(object):
    ...
    def __init__(self, x, y, dist=lambda x, y: numpy.linalg.norm(x - y), radius=1):
        ...
        self.normed_path_x = path[:, 0] / float(len(x))
        self.normed_path_y = path[:, 1] / float(len(y))
    ...
    @staticmethod
    def _interp_path(normed_path, target_length):
...
class MelCepstrumAligner(DTWAligner):
    def __init__(self, x, y, *args, **kwargs):
        ...
        super(MelCepstrumAligner, self).__init__(x, y, *args, **kwargs)
