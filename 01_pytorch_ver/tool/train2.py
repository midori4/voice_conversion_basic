...
if __name__ == "__main__":
    ...
    x_var = np.var(np.concatenate(x_train), axis=0, keepdims=True)
    y_mean = np.mean(np.concatenate(y_train), axis=0, keepdims=True)
    y_var = np.var(np.concatenate(y_train), axis=0, keepdims=True)
